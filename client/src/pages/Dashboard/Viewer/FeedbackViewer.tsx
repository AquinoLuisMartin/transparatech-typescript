  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        const res = await axios.get('/api/v1/feedback/categories', {
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="flex justify-end">
                    <button
                      type="submit"
                      disabled={loading}
                      className={`px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors font-medium flex items-center ${loading ? 'opacity-70 cursor-not-allowed' : ''}`}
                    >
                      {loading && (
